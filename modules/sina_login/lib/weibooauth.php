...
class SaeTOAuthV2 {
	...
	function getAuthorizeURL( $url, $response_type = 'code', $state = NULL, $display = NULL ) {
		...
		return $this->authorizeURL() . "?" . http_build_query($params, '', '&');//http_build_query($params);
	}
	...
	switch ($method) {
		case 'GET':
			//	REBOL edit
			$url = $url . '?' . http_build_query($parameters, '', '&');//http_build_query($parameters);
			return $this->http($url, 'GET');
		default:
			$headers = array();
			if (!$multi && (is_array($parameters) || is_object($parameters)) ) {
				$body = http_build_query($parameters, '', '&');//http_build_query($parameters);
			} else {
